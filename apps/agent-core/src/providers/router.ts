...
export class ModelRouter {
  ...
  constructor() {
    this.state.set("ollama", { weight: Number(process.env.NOVA_WEIGHT_OLLAMA ?? 5), failures: 0, openUntil: 0, healthy: true });
  ...
  async chat(messages: ChatMessage[]): Promise<ModelResponse> {
    ...
    for (const provider of ordered) {
      ...
      try {
        ...
        if (providerState.failures >= 3) {
          providerState.openUntil = Date.now() + 20_000;
        }
