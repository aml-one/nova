...
export async function POST(request: Request) {
  ...
  try {
    response = await fetch(`${getAgentBaseUrl(request)}/v1/setup/channels/signal/qrcodelink`, {
      method: "POST",
    ...
    return NextResponse.json(
      {
        error: `Could not reach agent-core Signal QR link endpoint: ${error instanceof Error ? error.message : String(error)}`
      },
      { status: 502 }
    );
  }
