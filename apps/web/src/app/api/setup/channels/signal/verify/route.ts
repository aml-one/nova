...
export async function POST(request: Request) {
  ...
  try {
    response = await fetch(`${getAgentBaseUrl(request)}/v1/setup/channels/signal/verify`, {
      method: "POST",
    ...
    return NextResponse.json(
      {
        error: `Could not reach agent-core Signal verification endpoint: ${error instanceof Error ? error.message : String(error)}`
      },
      { status: 502 }
    );
  }
  ...
  if (!response.ok) {
    const message =
      ...
      "Signal code verification failed";
    return NextResponse.json(
      {
        error: message,
