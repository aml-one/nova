...
async function readUpstream(response: Response): Promise<{ result?: unknown; error?: string }> {
  let raw = "";
  try {
  ...
  try {
    return JSON.parse(raw) as { result?: unknown; error?: string };
  } catch {
...
export async function POST(request: Request) {
  ...
  try {
    response = await fetch(`${getAgentBaseUrl(request)}/v1/system/update/apply`, {
      method: "POST",
